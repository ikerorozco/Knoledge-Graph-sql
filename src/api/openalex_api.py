...
def obtener_openalex_id_por_titulo(titulo):
    """
    Busca un paper por su título y devuelve su ID de OpenAlex.
...
def obtener_instituciones_por_openalex_id(openalex_id):
    """
    Obtiene todas las instituciones asociadas a un trabajo (work) usando su ID de OpenAlex.
    Retorna una lista de objetos Organization.
    Si no se encuentra el trabajo o hay un error, devuelve una lista vacía.
    """
    try:
        # Asegurarse de que el ID tiene el formato correcto
        if openalex_id.startswith("https://openalex.org/"):
            openalex_id = f"https://api.openalex.org/{openalex_id.replace('https://openalex.org/', '')}"
            
        api_url = f"{openalex_id}"
        response = requests.get(api_url, timeout=10)

        if response.status_code == 200:
            datos = response.json()
            
            # Lista para almacenar las organizaciones
            organizaciones = []
            
            # Extraer instituciones de las autorías
            for autor_data in datos.get("authorships", []):
                for inst_data in autor_data.get("institutions", []):
                    org_nombre = inst_data.get("display_name")
                    org_lugar = inst_data.get("country_code")
                    org_rdf_type = inst_data.get("type")
                    ...
                    # Evitar duplicados
                    if not any(org.nombre == org_nombre for org in organizaciones):
                        organizaciones.append(organizacion)
        ...
        print(f"Error al obtener instituciones para el ID '{openalex_id}': {e}")
        return []
